...
describe('GET /api/health', () => {
  it('reports ok when the database is reachable', async () => {
    const res = await request(createTestApp(prisma)).get('/api/health');
  ...
  it('reports degraded with 503 when the database is down', async () => {
    const res = await request(createTestApp(unreachablePrisma)).get('/api/health');
...
describe('error format', () => {
  it('returns NOT_FOUND in the standard error shape for unknown routes', async () => {
    const res = await request(createTestApp(prisma)).get('/api/nope');
